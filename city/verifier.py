...
def main(argv):
  ...
  vr.ReadSpace()
  M = vr.ReadInt(N - 1, min(200000, N * (N - 1) / 2))
  vr.ReadSpace()
  S = vr.ReadInt(1, N)
  vr.ReadSpace()
  T = vr.ReadInt(1, N)
  assert S != T, "S cannot be same with T"

  vr.ReadEndl()

  H = vr.ReadIntList(N, 0, 100000)
  ...
  for _ in range(M):
    ...
    assert (U, V) not in edges_set, 'pair of intersections connected by more than one road'
    edges_set.add((U,V))
  ...
  for i in range(1, N+1):
    assert root(i) == root(1), 'graph is disconnected'
