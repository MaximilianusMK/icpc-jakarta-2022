...
def main(argv):
  ...
  N, _, M = vr.ReadInt(1, 100), vr.ReadSpace(), vr.ReadInt(1, 100)
  vr.ReadEndl()
  ...
  for _ in range(N):
    X = vr.ReadInt(1, M)
    vr.ReadSpace()
    T = vr.ReadString(string.ascii_uppercase, 1, M)
    vr.ReadEndl()

    assert X + len(T) - 1 <= M, "A hint must cover a valid range"
    XT.add((X, T))

  assert len(XT) == N, "All hints must be unique"
