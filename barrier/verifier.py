...
def main(argv):
  ...
  for _ in range(N):
    X, Y = vr.ReadIntList(2, -10**9, 10**9)
    points.append((X, Y))

  assert len(points) == len(set(points)), 'There are 2 points at the same location'
  ...
  for pivot in points:
    ...
    for other in points:
      if other != pivot:
        ...
        m = simplify(dx, dy)
        gradients.append(m)
    assert len(gradients) == len(set(gradients)), 'There are 3 collinear points'
