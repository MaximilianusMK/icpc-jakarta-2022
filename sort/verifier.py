...
def main(argv):
  ...
  N, _, Q = vr.ReadInt(1, 100000), vr.ReadSpace(), vr.ReadInt(1, 100000)
  vr.ReadEndl()
  
  for _ in range(3):
    vr.ReadString(string.ascii_lowercase, N, N)
    vr.ReadEndl()
  
  for _ in range(Q):
    L, _, R = vr.ReadInt(1, N), vr.ReadSpace(), vr.ReadInt(1, N)
    vr.ReadEndl()
    assert L <= R, 'L must not be more than R'
